...
class KmerDataset(Dataset):
    ...
    def transform_kmerseq_table(self, X_kmer):
        ...
        for kmer in X_kmer:
            out_kmers[X_kmer.index(kmer)] = self.get_kmer_onehot(key)
...
def get_dataset(opt):
    ...
    dataloader = DataLoader(dataset, batch_size=opt.batch_size, shuffle=not opt.static, num_workers=0)
